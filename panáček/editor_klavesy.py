...
kurzor = pyglet.image.load('obrazky/kurzor.png')
kurzor.anchor_x = kurzor.width // 2
kurzor.anchor_y = kurzor.height // 2
vybrany = 0
vybrano = 1
kl = []
konec = False
...
def stisk_klavesy(klavesa, modifikatory):
    ...
    if klavesa == pyglet.window.key.D:
        ...
        konec = True
    elif klavesa == pyglet.window.key.SPACE:
        kl.append('sp')
...
def stisk(x, y, tlacitko, modifikatory):
    ...
    kliknuto = {'x' : pozice_mysi['x'], 'y' : pozice_mysi['y'], 'tlacitko' : tlacitko}
    if tlacitko == 2:
        if porad == True:
            porad = False
        else:
...
def pohyb(x, y, dx, dy):
    global pozice_mysi
    pozice_mysi = {'x' : pozice_mysi['x']+dx, 'y' : pozice_mysi['y']+dy}
...
def tik(dt):
    ...
    if 'sp' in kl:
        mapa[len(mapa)-pozice_mysi['y']//20-1][pozice_mysi['x']//20] = vybrano
    if 'l' in kl:
        pozice_mysi = {'x' : pozice_mysi['x']-5, 'y' : pozice_mysi['y']}
    if 'r' in kl:
        pozice_mysi = {'x' : pozice_mysi['x']+5, 'y' : pozice_mysi['y']}
    if 'u' in kl:
        pozice_mysi = {'x' : pozice_mysi['x'], 'y' : pozice_mysi['y']+5}
    if 'd' in kl:
        pozice_mysi = {'x' : pozice_mysi['x'], 'y' : pozice_mysi['y']-5}
    if pozice_mysi['x'] <= 0:
        pozice_mysi['x'] = 0
    if pozice_mysi['y'] <= 0:
        pozice_mysi['y'] = 0
    if pozice_mysi['x'] >= window.width:
        pozice_mysi['x'] = window.width
    if pozice_mysi['y'] >= window.height:
        pozice_mysi['y'] = window.height
    if konec:
        exit('SAVE')
...
def vykresli():
    ...
    sprites.append(pyglet.sprite.Sprite(kurzor,x=pozice_mysi['x'],y=pozice_mysi['y'],batch=batch))
    mapa.reverse()
    batch.draw()
    sprites.clear()
